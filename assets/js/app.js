...
Hooks.ComboboxHook = {
  mounted() {
    console.log("ComboboxHook mounted");
    this.selectedOptions = new Set();

    // Add click event listener to the options list
    const optionsList = document.getElementById(`${this.el.id.replace('-combobox', '')}-options`);
    ...
    // Add click event listener to the selected options container
    const selectedOptionsContainer = document.getElementById(`${this.el.id.replace('-combobox', '')}-selected-options`);
    selectedOptionsContainer.addEventListener('click', (event) => {
      if (event.target.closest('button')) {
        const value = event.target.closest('span').textContent.trim();
  ...
  findOptionElement(value) {
    const optionsList = document.getElementById(`${this.el.id.replace('-combobox', '')}-options`);
    return Array.from(optionsList.querySelectorAll('li')).find(li => li.querySelector('.block').textContent.trim() === value);
  },
  ...
  updateServer() {
    const filterName = this.el.id.replace('-combobox', '');
    const selectedOptionsArray = Array.from(this.selectedOptions);
    console.log("Updating server with:", filterName, selectedOptionsArray);
    this.pushEvent("update_filter", { [filterName]: selectedOptionsArray });
  }
};
